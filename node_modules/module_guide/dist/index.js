...
class GuideContent {
     ...
     * 引导线Guid
     */
    static GuideArrowGuid = "7697";
    /**
     * 引导线材质Guid
     */
    static GuideArrowMartialGuid = "13587";
    /**
     * 目的地特效Guid
     */
    static GuideWorldTargetEffectGuid = "21638";
    /**
     * 新手引导UI刷新时间
     */
 ...
 * @LastEditors: YuKun.Gao
 * @LastEditTime: 2023-03-23 18:36:39
 * @Description: file content
...
class GuideModuleC extends ModuleC {
     ...
     * 阶段完成广播
     */
    guideComplateAction = new Type.Action1();
    /**
     * 覆盖角色GameObject
     */
    _reSetCharGo;
     ...
     */
    setGuideArrowPointerInfo(guid, scale, isEffect, showPointerCount) {
    ...
    async resetAllGuideDB() {
        ...
        UI.UIManager.instance.getUI(GuideModuleView).hideAllItem();
    }
    /**
     * 重置指定的引导
     * @param guideId 引导id
    ...
    triggerGuide(guideId) {
        ...
        //UI.UIManager.instance.getUI(GuideModuleView).show();// getUI().show();
        UI.UIManager.instance.show(GuideModuleView);
        return true;
    }
    /**
     * Tick
 ...
 */
exports.GuideModuleUI_Generate = class GuideModuleUI_Generate extends UI.UIBehavior {
    mLeftMask = undefined;
    mTopMask = undefined;
 ...
 * @LastEditors: YuKun.Gao
 * @LastEditTime: 2023-03-23 18:34:39
 * @Description: file content
...
class GuideModuleView extends exports.GuideModuleUI_Generate {
     ...
     * 接收空间坐标
     */
    _outPixelPos = new Type.Vector2(0, 0);
    /**
     * 用于接收视口坐标
     */
    _outViewPos = new Type.Vector2(0, 0);
    /**
     * 当前引导绑定Target
     */
    _bindTarget = null;
    ...
    async showPointers(pos) {
        let guideModuleC = ModuleManager.getInstance().getModule(GuideModuleC);
        ...
        for (let i = Math.max(0, pos.length - guideModuleC.getGuideArrorShowMaxPointer()); i < pos.length; i++) {
            ...
            if (useCount >= cacheLength) {
                ...
                if (guideModuleC.getGuideArrowPointerIsEffect()) {
                    go = await Gameplay.Particle.asyncSpawn({ guid: guideModuleC.getGuideArrowPointerGuid(), replicates: false, transform: trans });
                    let eff = go;
                    eff.loop = true;
                    eff.play();
                }
                else {
                    go = await Core.GameObject.asyncSpawn({ guid: guideModuleC.getGuideArrowPointerGuid(), replicates: false, transform: trans });
                }
                go.setCollision(Type.PropertyStatus.Off);
                go.worldScale = guideModuleC.getGuideArrowPointerScale();
            ...
            if (pos.length > index + 1) {
                ...
                go.worldRotation = dir;
            }
            else {
                if (lastDir) {
                    go.worldRotation = lastDir.toRotation();
                }
            }
        ...
        for (let i = useCount; i < cacheLength; i++) {
            ...
            this._cachePointerGo[i].setVisibility(Type.PropertyStatus.Off);
        }
    }
    /**
     * 模块初始化
        ...
        if (this._guideArrow == null) {
            if (GuideContent.GuideArrowGuid != "") {
                Core.GameObject.asyncSpawnGameObject(GuideContent.GuideArrowGuid).then((go) => {
                    ...
                    this._guideArrow.setVisibility(Type.PropertyStatus.Off);
                    this._guideArrow.setCollision(Type.PropertyStatus.Off);
                });
            }
        }
        // 初始化目标点特效
        if (this._targetPosEff == null) {
            if (GuideContent.GuideWorldTargetEffectGuid) {
                Gameplay.Particle.asyncSpawnGameObject(GuideContent.GuideWorldTargetEffectGuid).then((go) => {
                    ...
                    this._targetPosEff.setVisibility(Type.PropertyStatus.Off);
                });
            }
        }
        // 设置隐藏mask
    ...
    hideAllItem() {
        ...
        for (let i = 0; i < this._cachePointerGo.length; i++) {
            if (ModuleManager.getInstance().getModule(GuideModuleC).getGuideArrowPointerIsEffect()) {
                let eff = this._cachePointerGo[i];
                eff.loop = false;
                eff.stop();
            }
    ...
    showMask(isShow) {
        ...
        if (isShow)
            ...
            UI.UIManager.instance.hide(GuideModuleView);
    }
    /**
     * 引导button点击后的回调
     */
    ...
    async setGuideArrowTargetPos(targetPos, callback, drawType, bUpdateArrow) {
        // 检查玩家是否到达目标点
        let char = await ModuleManager.getInstance().getModule(GuideModuleC).getCharGo();
        let guideModuleC = ModuleManager.getInstance().getModule(GuideModuleC);
        if (!bUpdateArrow) {
            //oTrace("设置到达目标点");
            // 设置到达目标点后的回调
            ...
            // 设置目标点特效
            if (this._targetPosEff) {
                if (this._targetPos != null && Type.Vector.equals(this._targetPos, targetPos))
                    return;
                this._targetPosEff.worldLocation = targetPos.clone(); //(new Type.Vector(0, 0, 0));
                this._targetPosEff.setVisibility(Type.PropertyStatus.On);
        ...
        if (char.worldLocation.clone().subtract(targetPos).length <= guideModuleC.getToTargetPosDistance()) {
            ...
            this._cachePointerGo.forEach(e => {
                e.setVisibility(Type.PropertyStatus.Off, true);
            });
            return;
        }
        // 设置箭头到达目标点
        if (this._drawType == 0) {
            if (this._guideArrow) {
                ...
                let chrLocal = new Type.Vector(char.worldLocation.x, char.worldLocation.y, char.worldLocation.z + guideModuleC.getGuideArrowStartPosOffsetZ());
                ...
                this._guideArrow.worldLocation = chrLocal.clone().add(offsetPos);
                this._guideArrow.worldRotation = offsetPos.toRotation().rotateVector(new Vector(90, 0, 0)).toRotation();
                this._guideArrow.setVisibility(Type.PropertyStatus.On);
                this._guideArrow.worldScale = new Type.Vector(scale, guideModuleC.getGuideArrowScaleY(), 1);
            }
        }
            ...
            if (this._lastUpdateTime == 0 || Date.now() - this._lastUpdateTime >= guideModuleC.getGuideArrowPointerUpdateInterval()) {
                let paths = await Gameplay.asyncFindPathToLocation(char.worldLocation, targetPos);
    ...
    lockMWUIWidget(target, callback, tips = "", isShowBtn = true) {
        ...
        UI.localToViewport(target.tickSpaceGeometry, Type.Vector2.zero, this._outPixelPos, this._outViewPos);
        ...
        if (this._bindTarget != target) {
            ...
            this._btnClickToTarget = null;
            //Input
            if (target instanceof UI.Button || target instanceof UI.StaleButton || target instanceof UI.MaskButton) {
                this._btnClickToTarget = () => {
        ...
        if (!this._outViewPos || this._outViewPos.equals(Type.Vector2.zero)) {
            return;
        }
        // 设置mask区域
        const viewportSize = Util.WindowUtil.getViewportSize();
        //const viewportSize = new Type.Vector2(5000, 5000);
        let targetSlotSize = target_slot.size;
        // 设置Top
        this.mTopMask.slot.size = new Type.Vector2(viewportSize.x, this._outViewPos.y);
        // 设置Buttom
        this.mButtomMask.slot.position = new Type.Vector2(0, this._outViewPos.y + targetSlotSize.y);
        this.mButtomMask.slot.size = new Type.Vector2(viewportSize.x, viewportSize.y - this.mButtomMask.slot.position.y);
        // 设置Left
        this.mLeftMask.slot.position = new Type.Vector2(0, this.mTopMask.slot.size.y);
        this.mLeftMask.slot.size = new Type.Vector2(this._outViewPos.x, viewportSize.y - this.mTopMask.slot.size.y - this.mButtomMask.slot.size.y);
        // 设置Right
        this.mRightMask.slot.position = new Type.Vector2(this._outViewPos.x + targetSlotSize.x, this.mTopMask.slot.size.y);
        this.mRightMask.slot.size = new Type.Vector2(viewportSize.x - this.mButtomMask.slot.position.x, viewportSize.y - this.mTopMask.slot.size.y - this.mButtomMask.slot.size.y);
        //设置手指
        this.mBtnHand.slot.position =
            new Type.Vector2(this._outViewPos.x + targetSlotSize.x / 2 - this.mBtnHand.slot.size.x / 2, this._outViewPos.y + targetSlotSize.y / 2);
        this.mTextHand.text = (tips);
        let size = this.mTextHand.desiredSize;
        let btnHandPos = this.mBtnHand.slot.position.clone(); //.addition(Type.Vector2.zero);
        let btnHandSize = this.mBtnHand.slot.size.clone(); //.addition(Type.Vector2.zero);
        if (this._outViewPos.y < viewportSize.y / 2) {
            //在屏幕上方
            if (this._outViewPos.x < viewportSize.x / 2) {
                this.mTextHand.slot.position =
                    new Type.Vector2(btnHandPos.x, btnHandPos.y + btnHandSize.y);
            }
            else {
                this.mTextHand.slot.position =
                    new Type.Vector2(btnHandPos.x + (btnHandSize.x - size.x), btnHandPos.y + btnHandSize.y);
            }
        }
        else {
            //在屏幕下方
        ...
        // 设置button区域
        this.mBtn.slot.position = new Type.Vector2(this._outViewPos.x, this._outViewPos.y);
        this.mBtn.slot.size = new Type.Vector2(target_slot.size.x, target_slot.size.y);
        this.showMask(true);
        if (isShowBtn)
    ...
    static newGuideTaskStage(type, condition, runFuncs = [], uiConditionRes = null) {
        ...
        switch (type) {
            case GuideState.WaitUI:
                ...
                break;
            case GuideState.WaitToPos:
                if (condition instanceof Type.Vector)
...
class GuideInfo {
    ...
    handle(dt) {
        ...
        if (this.curState == GuideState.WaitToPos) {
            UI.UIManager.instance.getUI(GuideModuleView).setGuideArrowTargetPos(this.taskStage[0].toWorldPos, null, 0, true);
            return;
        }
        /**
         * 验证是否完成自定义条件
    ...
    pullTask() {
        ...
        if (this.taskStage.length > 0) {
            ...
            if (task.type == GuideState.WaitUI) {
                ...
                UI.UIManager.instance.getUI(GuideModuleView).showMask(false);
            }
            //拉取目标点任务
    ...
    beginUITask(widget) {
        ...
        UI.UIManager.instance.getUI(GuideModuleView).lockMWUIWidget(widget, this.onUITaskCallback.bind(this), this.taskStage[0].tips, this.taskStage[0].uiCheckCondition == null);
    }
    /**
     * 开始到目标点引导
     * @param toPos 目标点
    ...
    beginToWorldPosTask(toPos, drawType) {
        ...
        UI.UIManager.instance.getUI(GuideModuleView).showMask(false);
        UI.UIManager.instance.getUI(GuideModuleView).setGuideArrowTargetPos(toPos, this.onWorldToPosTaskCallback.bind(this), drawType, false);
    }
}
...
class GuideDataHelper extends Subdata {
    onGuideChangeAction = new Type.Action();
    complateGuide = null;
    /**
     * 已经完成的引导
     */
...
class GuideModuleS extends ModuleS {
    ...
    guideIsComplate(guideId, playerId) {
        return DataCenterS.getInstance().getData(playerId, GuideDataHelper).guideIsComplate(guideId);
    }
}
